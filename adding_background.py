...
def redrawTheWindow(currentScore, currentTime, c, pl, pre, level):
    ...
    for i in range(6):
        pygame.draw.rect(
            window, greenBackground,
            (0, 2 * i * (winHeight // 11), winWidth, winHeight // 11)
        )
    for i in vil:
        i.hb = (i.x + 17, i.y + 19, 29, 60)
        pygame.draw.rect(window, redBackground, i.hb, 2)
        i.draw(window)

    for i in spike:
        i.hb = (i.x + 20, i.y + 70, 100, 58)
        pygame.draw.rect(window, redBackground, i.hb, 2)
        i.draw(window)
    for i in coins:
        pygame.draw.rect(window, redBackground, i.hb, 2)
        i.draw(window)
    man.draw(window)
    pygame.draw.rect(window, redBackground, man.hb, 2)
